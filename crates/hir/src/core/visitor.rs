    ...
    V: Visitor<'db> + ?Sized,
{
    ...
    if let Some(ty) = func.ret_type_ref___tmp(ctxt.db) {
        ctxt.with_new_ctxt(
            |span| span.ret_ty(),
            |ctxt| {
    ...
    V: Visitor<'db> + ?Sized,
{
    ...
    if let Some(ty) = alias.type_ref___tmp(ctxt.db).to_opt() {
        ctxt.with_new_ctxt(
            |span| span.ty(),
            |ctxt| {
                visitor.visit_ty(ctxt, ty);
...
pub fn walk_impl<'db, V>(
    ...
    V: Visitor<'db> + ?Sized,
{
    if let Some(ty) = impl_.type_ref___tmp(ctxt.db).to_opt() {
        ctxt.with_new_ctxt(
            |span| span.target_ty(),
            |ctxt| {
                visitor.visit_ty(ctxt, ty);
    ...
    V: Visitor<'db> + ?Sized,
{
    ...
    if let Some(ty) = impl_trait.type_ref___tmp(ctxt.db).to_opt() {
        ctxt.with_new_ctxt(
            |span| span.ty(),
            |ctxt| {
                visitor.visit_ty(ctxt, ty);
    ...
    V: Visitor<'db> + ?Sized,
{
    ...
    if let Some(ty) = const_.type_ref___tmp(ctxt.db).to_opt() {
        ctxt.with_new_ctxt(
            |span| span.ty(),
            |ctxt| {
                visitor.visit_ty(ctxt, ty);
    ...
    V: Visitor<'db> + ?Sized,
{
    ...
    if let Some(ty) = field.field_type_ref___tmp().to_opt() {
        ctxt.with_new_ctxt(
            |span| span.ty(),
            |ctxt| {
                visitor.visit_ty(ctxt, ty);
